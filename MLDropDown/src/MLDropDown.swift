...
public typealias CellConfigurationClosure = (Index, String, MLDropDownCell) -> Void
...
public final class MLDropDown: UIView {
	...
	public static weak var VisibleDropDown: MLDropDown?

	//MARK: UI
	fileprivate let dismissableView = UIView()
	fileprivate let tableViewContainer = UIView()
	fileprivate let tableView = UITableView()
	fileprivate var templateCell: MLDropDownCell!
	...
	*/
	public var animationEntranceOptions: UIView.AnimationOptions = MLDropDown.animationEntranceOptions
	
	/**
	The option of the hide animation. Only change the caller. To change all drop down's use the static var.
	*/
	public var animationExitOptions: UIView.AnimationOptions = MLDropDown.animationExitOptions
	...
	*/
     ...
     */
	public var cellNib = UINib(nibName: "MLDropDownCell", bundle: bundle) {
		didSet {
			tableView.register(cellNib, forCellReuseIdentifier: DPDConstant.ReusableIdentifier.MLDropDownCell)
			templateCell = nil
			reloadAllComponents()
		}
	}
  ...
  fileprivate static var bundle: Bundle {
    ...
    #else
    return Bundle(for: MLDropDownCell.self)
    #endif
  }
...
extension MLDropDown {
	...
	fileprivate func fittingWidth() -> CGFloat {
		if templateCell == nil {
			templateCell = (cellNib.instantiate(withOwner: nil, options: nil)[0] as! MLDropDownCell)
		}
...
extension MLDropDown {
	...
	*/
    ...
    public func show(onTopOf window: UIWindow? = nil, beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
		if self == MLDropDown.VisibleDropDown && MLDropDown.VisibleDropDown?.isHidden == false { // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue when continuous taping on button - Kartik Patel - 2016-12-29
			return (true, 0)
		}

		if let visibleDropDown = MLDropDown.VisibleDropDown {
			visibleDropDown.cancel()
		}

		willShowAction?()

		MLDropDown.VisibleDropDown = self
	...
	public func hide() {
		...
		if self == MLDropDown.VisibleDropDown {
			/*
			...
			*/
			MLDropDown.VisibleDropDown = nil
		}
...
extension MLDropDown: UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: DPDConstant.ReusableIdentifier.MLDropDownCell, for: indexPath) as! MLDropDownCell
	...
	fileprivate func configureCell(_ cell: MLDropDownCell, at index: Int) {
